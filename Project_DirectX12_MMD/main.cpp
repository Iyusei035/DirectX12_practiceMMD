...
LRESULT WindowProcedure(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	if (msg == WM_DESTROY) {//ウィンドウが破棄されたら呼ばれます
		PostQuitMessage(0);//OSに対して「終了」と伝える
		return 0;
	}
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	depthResDesc.DepthOrArraySize = 1;//配列でもテクスチャでもない
	depthResDesc.Format = DXGI_FORMAT_D32_FLOAT;//深度値書き込み用フォーマット
	depthResDesc.SampleDesc.Count = 1;//1ピクセルあたり1つ
...
#pragma pack()//一バイトパッキング終了
	...
	matDescHeapDesc.NumDescriptors = materialNum * 5;//マテリアルの数
	...
	//テクスチャ用
	descTblRange[2].NumDescriptors = 4;//テクスチャ3つ
	descTblRange[2].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;//種別はテクスチャ
	descTblRange[2].BaseShaderRegister = 0;//1番スロットから
	...
	rootparam[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;				//全てのシェーダーから見える
	...
	samplerDesc[1].BorderColor = D3D12_STATIC_BORDER_COLOR_TRANSPARENT_BLACK;//ボーダーの時は黒
	samplerDesc[1].Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;//補間しない(ニアレストネイバー)
	samplerDesc[1].MaxLOD = D3D12_FLOAT32_MAX;//ミップマップ最大値
	samplerDesc[1].MinLOD = 0.0f;//ミップマップ最小値
	samplerDesc[1].ComparisonFunc = D3D12_COMPARISON_FUNC_NEVER;//オーバーサンプリングの際リサンプリングしない？
	samplerDesc[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//ピクセルシェーダからのみ可視
	...
	(
		XM_PIDIV2,//画角
		static_cast<float>(window_width) / static_cast<float>(window_height),//アスペクト
		1.0f,//ニアクリップ
		100.0f//ファークリップ
	);
	...
	SceneMatrix* mapMatrix = nullptr;//マップ先のポインタ
